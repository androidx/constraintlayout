...
public class VpGraphCoreOrig {
    private static final String FPS_STRING = "onDraw";

    List<Data> mPlots = new ArrayList<>();
    ...
    static class Data {
        ...
        void plot(Canvas canvas, VpGraphCoreOrig graph, int w, int h) {
            path.reset();
    ...
    public VpGraphCoreOrig(UiDelegate uiDelegate) {
    ...
    public void addChannel(String str) {
        mPlots.add(new Data(str));
    }

    public void setGraphFPS(boolean on) {
        mGraphSelfFps = on;
        if (on) {
            mPlots.add(new Data(FPS_STRING));
    ...
    private void drawGrid(Canvas canvas, int w, int h) {
        ...
        for (float y = minY; y <= maxY; y += ticksY) {
            ...
            if ((count & 1) == 1 && (y + ticksY) < maxY) {
                ...
                canvas.drawText(str, axisLeft - mBounds.width() - txtPad, yp, mGridPaint);
            }
            count++;
        }
    }
    ...
    void drawTouchLine(Canvas canvas, int w, int h) {
        ...
        int no_of_plots = mPlots.size();
        for (int i = 0; i < no_of_plots; i++) {
            Data plot = mPlots.get(i);
            ...
            float minVGap = mBounds.height();
            float force = 0;
            for (int j = 0; j < no_of_plots; j++) {
                if (i == j) {
                    continue;
                }
                float yOther = mPlots.get(j).lastLabelYPos;
                float dist = Math.abs(plot.lastLabelYPos - yOther);
                if (dist < minVGap * 2) {
                    float dir = Math.signum(yPos - yOther);
                    force = ((dir > 0) ? minVGap : -minVGap) + minVGap / (0.1f + dist);
                }
            }
