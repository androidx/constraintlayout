...
public class CycleGraph  extends  JPanel {
    ...
    public CycleGraph(CycleModel cycle) {
        myCycle = cycle;
        p.addElement(mGrid);
        p.addElement(mAxis);
        ...
        waveGen(0);
        calcRange();
        cycle.addModelListener((e)->updateGraph(e));
    ...
    private void updateGraph(CycleModel model) {
        setCycle(0, model.values[model.POS],
                model.values[model.PERIOD],
                model.values[model.AMP],
                model.values[model.OFFSET],
                model.values[model.PHASE],
                model.selected,
                model.mModeValue);
    }
    ...
    public static void save(ActionEvent e, CycleGraph graph) {
    ...
    public void setCycle(int n, double[] pos, double[] period, double[] amplitude, double[] offset,double []phase,
                         int selected, int curveType) {
        double[] t = new double[pos.length];
        double[][] v = new double[pos.length][3];
        ...
        for (int i = 0; i < x.length; i++) {
            ...
            // TODO add Phase mMonotoneSpline.getPos(v, 2);
            y1[i] = mOscillator.getValue(x[i],0) * amp + off;
    ...
    float getComputedValue(float v) {
        ...
        double phase = 0; // TODO add phase mMonotoneSpline.getPos(v, 2);
        return (float) (mOscillator.getValue(v, phase) * amp + off);
    }
    ...
    public static void main(String[] args) {
        ...
        frame.setContentPane(new CycleGraph(model
                ));
        frame.setTitle("Simple Editor");
        Preferences pref = Preferences.userNodeForPackage(CycleGraph.class);
        Rectangle pos = new Rectangle(200, 100, 700, 600);
        if (pref != null && pref.getInt("base_x", -1) != -1) {
            pos.x = pref.getInt("base_x", pos.x);
            pos.y = pref.getInt("base_x", pos.y);
            pos.width = pref.getInt("base_x", pos.width);
            pos.height = pref.getInt("base_x", pos.height);
        }
        frame.setBounds(pos);
        frame.addComponentListener(new ComponentAdapter() {
            public void componentMoved(ComponentEvent evt) {
                pref.putInt("base_x", pos.x);
                pref.putInt("base_x", pos.y);
                pref.putInt("base_x", pos.width);
                pref.putInt("base_x", pos.height);
            }
        });
