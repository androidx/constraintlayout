...
//==================== MotionScene =========================
    ...
    public static void parseJSON(String content, State state, CorePixelDp dpToPixels, LayoutVariables layoutVariables) throws CLParsingException {
        try {
            CLObject json = CLParser.parse(content);
            ArrayList<String> elements = json.names();
            if (elements == null) {
                ...
                switch (elementName) {
                    ...
                    case "Helpers":
                        if (element instanceof CLArray) {
                            parseHelpers(state, dpToPixels, layoutVariables, (CLArray) element);
                        }
                        break;
                    case "Generate":
                        if (element instanceof CLObject) {
                            parseGenerate(state, dpToPixels, layoutVariables, (CLObject) element);
                        }
                        break;
                    default:
                        if (element instanceof CLObject) {
                            ...
                            if (type != null) {
                                ...
                                parseWidget(state, dpToPixels, layoutVariables, elementName, (CLObject) element);
                            }
    ...
    static void parseHelpers(State state, CorePixelDp dpToPixels, LayoutVariables layoutVariables, CLArray element) throws CLParsingException {
            ...
            if (helper instanceof CLArray) {
                ...
                if (array.size() > 1) {
                    switch (array.getString(0)) {
                        case "hChain":
                            parseChain(ConstraintWidget.HORIZONTAL, state, dpToPixels, layoutVariables, array);
                        case "vChain":
                            parseChain(ConstraintWidget.VERTICAL, state, dpToPixels, layoutVariables, array);
                        case "hGuideline":
                            parseGuideline(ConstraintWidget.HORIZONTAL, state, dpToPixels, array);
                        case "vGuideline":
                            parseGuideline(ConstraintWidget.VERTICAL, state, dpToPixels, array);
                    }
                }
            }
        }
    ...
    static void parseGenerate(State state, CorePixelDp dpToPixels, LayoutVariables layoutVariables, CLObject json) throws CLParsingException {
        ArrayList<String> elements = json.names();
        if (elements == null) {
            return;
        }
        for (String elementName : elements) {
            ...
            if (arrayIds != null && element instanceof CLObject) {
                for (String id : arrayIds) {
                    parseWidget(state, dpToPixels, layoutVariables, id, (CLObject) element);
                }
            }
        }
    }

    static void parseChain(int orientation, State state, CorePixelDp dpToPixels, LayoutVariables margins, CLArray helper) throws CLParsingException {
    ...
    static void parseGuideline(int orientation, State state, CorePixelDp dpToPixels, CLArray helper) throws CLParsingException {
        ...
        parseGuidelineParams(orientation, state, dpToPixels, guidelineId, (CLObject) params);
    }
        ...
        for (String constraintName : constraints) {
            switch (constraintName) {
                case "start":
                    ...
                    guidelineReference.start(dpToPixels.toPixels(margin));
                    break;
                case "end":
                    margin = state.convertDimension(params.getFloat(constraintName));
                    guidelineReference.end(dpToPixels.toPixels(margin));
                    break;
                case "percent":
    ...
    static void parseWidget(
        ...
        for (String constraintName : constraints) {
            switch (constraintName) {
                case "width":
                    reference.setWidth(parseDimension(element, constraintName, state, dpToPixels));

                    break;
                case "height":
                    reference.setHeight(parseDimension(element, constraintName, state, dpToPixels));
                    break;
                case "center":
                ...
                case "custom":
                    ...
                    break;
                default:
                    parseConstraint(state, dpToPixels, layoutVariables, element, reference, constraintName);
    ...
    static void parseConstraint(
        ...
        if (constraint != null && constraint.size() > 1) {
            ...
            if (constraint.size() > 2) {
                ...
                margin = state.convertDimension(dpToPixels.toPixels(margin));
            }
            if (constraint.size() > 3) {
                CLElement arg2 = constraint.getOrNull(3);
                marginGone = layoutVariables.get(arg2);
                marginGone = state.convertDimension(dpToPixels.toPixels(margin));
            }
